...
def scatter(self, nbins=10, show=False, savefig=None):
    ...
    ax.axhline(avg, label="Average", color=utils.METHODS["avg"]["color"])
    ax.axhline(
        avg + std,
        label=fr"1$\sigma$ deviation",
        linestyle="dashed",
        color=utils.METHODS["std"]["color"],
    )
    ax.axhline(
        avg - std, linestyle="dashed", color=utils.METHODS["std"]["color"]
    )
    ...
    ax.scatter(
        ...
        np.asarray(self)[cur],
        label=m,
        marker=utils.METHODS[m]["marker"],
        s=80,
        facecolors=fcol,
        edgecolors=utils.METHODS[m]["color"],
    )
    ax.errorbar(
        x[cur],
        np.asarray(self)[cur],
        yerr=np.asarray(self.error)[cur],
        c=utils.METHODS[m]["color"],
        linestyle="",
    )
    ...
    ax.set_ylabel(utils.PLOTTING["LABELS"][self.__name__])
    ax.legend(loc="best", ncol=2)
...
def hist(self, nbins=10, show=False, savefig=None):
    ...
    plt.xlabel(utils.PLOTTING["LABELS"][self.__name__])
